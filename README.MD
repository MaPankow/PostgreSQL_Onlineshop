...
Link zur Aufgabe: https://github.com/bergemann-hoehne/Datenbank-Wochenprojekte/tree/main/09_Wochenprojekt

**Das Projekt startete ich am 13.09.2024**
In diesem Projekt möchte ich eine Postgres-Datenbank für einen Onlineshop anlegen und weitere SQL-Befehle für Abfragen üben. 

Es ist viel Text und wer ein bisschen überfliegen möchte, kann sich ganz am Ende mein "ERD nachher" anschauen, es ist ein physisches Entity-Relationship-Diagramm, das die Datenbank abbildet. 
Viel Spaß!

Ein logisches ERD ist bereits vorgegeben:
![logisches Entity-Relationship-Diagram als Aufgabenstellung](/Onlineshop_Diagramm.png)
Für mein Beispiel wähle ich einen Online-Comicshop und nenne ihn World of Comics.
Für weitere Überlegungen und um Datentypen zu recherchieren und festzuhalten, lege ich noch ein physisches ERD an, mit dem ich im Verlauf des Projektes arbeite. Unter der Überschrift "ERD vorher" ist es zu finden.
...
## Implementierung der ersten Tabelle
Da ich Diagramm vorgegeben bekommen habe, möchte ich es zunächst richtig verstehen, Unklarheiten klären und überlegen, welche Datentypen hier richtig wären, bevor ich direkt implementiere. 
Notiz: Ich war so neugierig, dass ich in meinem ERD nur die erste Tabelle angelegt und direkt implementiert habe. Danach habe ich mich zusammengerissen.
...
-- Hat man sich, wie ich, verschrieben, kann man auch jederzeit den Namen der Spalte ändern. Bei einer genaueren Suche habe ich auch gefunden, wie es geht. Wegen o. g. Erklärungen würde ich jedoch für den Fall, dass ich irgendwann eine andere Methode hinzufügen und statt Paypal benutzen will, in jedem Fall einen neuen Wert dafür ins Enum eintragen.
...
-- 17.09.2024
-- Best Practice ist es, die Tabellennamen in der Mehrzahl zu implementieren. Ideal ist es, das schon im Vorhinein festzulegen. Man kann die Tabellen aber auch umbenennen. 
...
Zur Tabelle Lager ist anzumerken, dass ich mir gerade Gedanken gemacht habe, wie das mit der Lagerung überhaupt funktioniert. Grobe Recherchen ergaben, dass es die Möglichkeit einer zentralen oder dezentralen Lagerung gibt, mit dem Unterschied, dass man bei der zentralen Lagerung ein einziges Lager hat. (Spoiler: dann bräuchte ich jetzt aber irgendwie keine Tabelle für die Lager). Tatsächlich habe ich auch gefunden, dass für Onlineshops die dezentrale Lagerung benutzt wird. Und weil ein Comicshop vielleicht zumindest am Anfang nicht so ein Riesengeschäft ist, dass man eigene Lagerhäuser unterhalten könnte, habe ich noch recherchiert, ob man einzelne Lagerräume anmieten kann. Kann man. Da bin ich aber beruhigt!

Fragen tun sich bei mir in der Beziehung zwischen Lager und Produkten auf. Im vorgegebenen Diagramm gibt es eine One-to-Many-Beziehung zwischen der Tabelle "Lager" und der Tabelle "Produkte". Wäre dies nun ein Auftrag für einen echten Shop, würde ich die BetreiberInnen fragen. Ich gehe nun davon aus, dass wir mehrere Lager haben und dass, aufgrund der kurzen Lieferwege, die der Vorteil eines dezentralen Lagerungssystems sind, möglichst in mehreren Lagern dasselbe Produkt vorrätig sein soll. Ein Produkt kann in mehreren Lagern sein und in einem Lager sind mehrere verschiedene Produkte. Das ist dann eine *Many-to-Many-Beziehung*. 
Hier sollte also noch eine *Hilfstabelle* implementiert werden. Das ist die Produkt-im-Lager-Tabelle. 
Die Tabelle bekommt eine  Spalte, die die Stückzahl eines Produktes in einem der Lager anzeigt. Zudem füge ich noch eine Spalte letzte_lieferung_am ein, die einen Timestamp erhält, der bei jeder Belieferung aktualisiert wird. So kann man sehen, welche Produkte zu Ladenhütern werden.

## ERD vorher
Hier ist mein physisches ERD, Stand 17.09.2024:

![physisches Entity-Relationship-Diagram vorher](/ERD-physisch.png)
...
-- ich tragen eine Bestellung ein:
...
-- Ich habe beide Befehle gemeinsam abgesendet, doch eigentlich codet man die 2 nicht da rein. In diesen Foreign Key soll die generierte id, die die Bestellung tatsächlich in ihrer Tabelle hat, eingetragen werden. 
-- Wenn ich später in der Anwendung eine Bestellung abschicke, passieren dort mehrere Befehle hintereinander, in denen erst die Bestellung zusammen mit der id_benutzer in die Tabelle Bestellungen eingetragen wird. Sie bekommt eine Id_Bestellung, die durch die Anwendung ausgelesen und dann an die Tabelle Bestellhistorien übergeben wird. Der Timestamp wird in beiden Tabellen automatisch erzeugt.
...
Bis jetzt hat User1 einfach nur bestellt und einen Preis eingegeben. 
Darum wird erst mal der Bestellvorgang mit einer Tabelle für die Dinge, die bestellt werden können, komplett gemacht.
...
-- außerdem haben die Produkte einen verschiedenen Mehrwertsteuersatz, je nachdem, ob es sich um Bücher (7 %) oder Figuren und Spiele handelt (19 %)

ALTER TABLE produkte ADD COLUMN mwst_satz NUMERIC;
-- damit man damit auch rechnen kann, ist es jetzt ein NUMERIC
```

19.09.2024: Nach weiteren Überlegungen komme ich zu dem Ergebnis, dass ich in dieser Tabelle den Brutto- und Nettopreis sowie den Betrag der Mehrwertsteuer haben möchte, damit die einzelnen Beträge in der Tabelle Bestellungen einfach nur aufaddiert werden können.
...
-- es gibt zwei Bestellungen, denen ich jetzt Produkte zuordne
...
Ich nenne die Spalte für die Kapazität um in kapazitaet_m2, was ausdrückt, dass die Quatdratmeterzahl eingetragen wird, und gebe ihr den Datentyp SMALLINT, weil ich nicht glaube, dass ein Lagerraum über 32,7 Quadratkilometer groß sein kann.
```sql
CREATE TABLE lager (
...
### 6. Lager auffüllen
23.09.2024: Mein Comicshop hat fünf Lager über Deutschland vertreilt. Hier ist noch mal die Tabelle:
```sql
world_of_comics=# select* FROM lager;
 id_lager | standort  | kapazitaet_m2 
----------+-----------+---------------
...
-- Jetzt möchte ich in jedem Lager die Produkte um 10 Stück auffüllen, von denen nur noch 5 oder weniger vorhanden sind. Ich lege mit Hamburg los. Um zu sehen, ob die Transaktion funktioniert, lasse ich mir kurz noch mal alles anzeigen.

SELECT p.name, p.autor, l.standort, pl.stueckzahl 
FROM produkt_im_lager pl
INNER JOIN produkte p ON pl.fid_produkt = p.id_produkt
INNER JOIN lager l ON pl.fid_lager = l.id_lager
WHERE l.standort = 'Hamburg';

                name                 |     autor      | standort | stueckzahl 
-------------------------------------+----------------+----------+------------
 Frieren 10                          | Yamada/Abe     | Hamburg  |         20
 Der Verkehrte Himmel                | Ross           | Hamburg  |          5
 Kinderland                          | Mawil          | Hamburg  |          2
 Sonne und Beton                     | Lobrecht/Haus  | Hamburg  |          0
 Wie ein leeres Blatt                | Bloulet/Bagieu | Hamburg  |          1
 Naruto Shippuden - Manga-Figur 17cm | Anime Heroes   | Hamburg  |         10
(6 rows)

-- Oh, hier muss dringend was getan werden!!! Ich probiere erst mal etwas aus.

UPDATE produkt_im_lager SET stueckzahl = stueckzahl + 10
    WHERE fid_lager = 1 AND stueckzahl BETWEEN 0 AND 5;

-- Moment mal! (noch nicht ausgeführt) Kann man nicht einfach alle Lager super leicht auf einmal auffüllen?
UPDATE produkt_im_lager SET stueckzahl = stueckzahl + 10
    WHERE stueckzahl BETWEEN 0 AND 5;

-- Man kann. Zwei Stichproben:

world_of_comics=# SELECT p.name, p.autor, l.standort, pl.stueckzahl 
FROM produkt_im_lager pl
INNER JOIN produkte p ON pl.fid_produkt = p.id_produkt
INNER JOIN lager l ON pl.fid_lager = l.id_lager
WHERE l.standort = 'Hamburg';
...
-------------------------------------+----------------+----------+------------
 Frieren 10                          | Yamada/Abe     | Hamburg  |         20
 Naruto Shippuden - Manga-Figur 17cm | Anime Heroes   | Hamburg  |         10
 Der Verkehrte Himmel                | Ross           | Hamburg  |         15
 Kinderland                          | Mawil          | Hamburg  |         12
 Sonne und Beton                     | Lobrecht/Haus  | Hamburg  |         10
 Wie ein leeres Blatt                | Bloulet/Bagieu | Hamburg  |         11
(6 rows)
...
INNER JOIN produkte p ON pl.fid_produkt = p.id_produkt
INNER JOIN lager l ON pl.fid_lager = l.id_lager
WHERE l.standort = 'Köln';
                name                 |     autor      | standort | stueckzahl 
-------------------------------------+----------------+----------+------------
 Frieren 10                          | Yamada/Abe     | Köln     |         24
 Der Verkehrte Himmel                | Ross           | Köln     |         10
 Sonne und Beton                     | Lobrecht/Haus  | Köln     |         10
 Wie ein leeres Blatt                | Bloulet/Bagieu | Köln     |         20
 Naruto Shippuden - Manga-Figur 17cm | Anime Heroes   | Köln     |         10
 Kinderland                          | Mawil          | Köln     |         10
(6 rows)
...
25.09.2024: Ich möchte User 2, die/den ich ganz frisch angelegt habe (Toll, dass man bei zwei KundInnen fünf Lager unterhalten kann, nicht wahr?), etwas bestellen lassen. Hierfür soll aus dem Lager, dass User 2 am nächsten liegt, der Comic entnommen werden. Die Stückzahl wird reduziert.
Hier könnte man z. B. sie Stückzahl eines Produktes im Lager verändern, WHERE eine Bestellung aus einem bestimmten Bereich kommt. 

Ich fange mit der Recherche an, welche Postleitzahlen wohl im Dunstkreis des Lagers in Berlin liegen könnten. Eine sehr grobe Recherche zu Übungszwecken ergab, dass die Postleitzahlen mit 0 und 1 am Anfang infrage kämen. (Brandenburg, Berlin, Teile Sachsens und Sachsen-Anhalts etc.) Ich denke an eine LIKE-Abfrage an einem Punkt. Hier ist der Prozess und was ich dabei herausgefunden habe:

```sql

-- Ich weiß, dass die Bestellung nach Berlin geschickt werden soll. Mal ins Lager reinschauen:
SELECT * FROM lager WHERE standort = 'Berlin';
...
-- User 2 bestellt sich "Der verkehrte Himmel".
SELECT * FROM produkte WHERE name = 'Der Verkehrte Himmel';

 id_produkt |         name         |     isbn      | preis_netto | autor |    verlag    | mwst_satz | preis_brutto | mwst_betrag 
------------+----------------------+---------------+-------------+-------+--------------+-----------+--------------+-------------
          2 | Der Verkehrte Himmel | 9783964451088 |       26.17 | Ross  | avant-verlag |         7 |        28.00 |        1.83
(1 row)

-- ich lege eine leere Bestellung an, die später geupdatet werden soll. Doch für gewisse Abfragen braucht es zunächst die id_bestellung
INSERT INTO bestellungen (fid_benutzer, gesamtpreisnetto, mwst, gesamtpreisbrutto) VALUES
(2, 0, 0, 0);

SELECT * FROM bestellungen WHERE fid_benutzer = 2 ORDER BY bestelldatum DESC LIMIT 1;

 id_bestellung |        bestelldatum        | fid_benutzer | gesamtpreisnetto | mwst | gesamtpreisbrutto 
---------------+----------------------------+--------------+------------------+------+-------------------
             6 | 2024-09-25 16:22:04.200982 |            2 |             0.00 | 0.00 |              0.00
(1 row)

-- gibt die neueste Bestellung von User 2 aus, mit der id_bestellung, die für die Transaktion benötigt wird, in meinem Fall Nummer 6
-- mir ist bewusst, dass die Datenbank mit keiner Anwendung verknüpt ist. Daten, die ich in die Transaktion einfüge, würden über das Backend mittels einer Anwendung eingesetzt. Dieses holt sich die Daten, indem es die Queries an anderer Stelle in den Code einprogrammiert bekommt, die ich hier einzeln erstelle. 
-- an die id_produkt kommt die Anwendung dadurch, dass User2 die Produkte angezeigt bekommt und dieses auswählt
-- beim Schreiben der Transaktion fiel mir auf, dass nicht nur die id_bestellung und das Produkt bekannt sein müssen, sondern auch die id_lager, damit die Stückzahl des richtigen Produktes im richtigen Lager geändert werden kann (In dem Fall bestellt User2 das Produkt nach Berlin)
-- nach vielem Überlegen, wie man über die Postleitzahl an das Lager kommt, stelle ich fest, es braucht dafür noch eine oder mehrere Spalten in der Tabelle Lager
-- das Testen mit der Transaktion ergibt also ein Ergebnis, dass eine Tabelle verbessert werden muss. Ich kümmere mich zunächst um die Transaktion und code die id_lager 2 rein

INSERT INTO bestellhistorien (fid_benutzer, fid_bestellung) VALUES (2, 6);
-- die Bestellhistorie muss immer mit ausgefüllt werden

INSERT INTO bestellpositionen (fid_bestellung, fid_produkt, menge) VALUES (6, 2, 1);
-- die id_bestellposition brauche ich ebenfalls für die Transaktion

SELECT * FROM bestellpositionen WHERE fid_bestellung = 6;
 id_bestellposition | fid_bestellung | fid_produkt | menge 
--------------------+----------------+-------------+-------
                  6 |              6 |           2 |     1
(1 row)

-- in der Transaktion fülle ich erst die Tabelle bestellungen aus mit den Preisen des Produktes verrechnet mit der Menge in der Tabelle Bestellpositionen. Sollte eine Bestellung mehrere Bestellpositionen enthalten, muss die Transaktion mehrmals ausgeführt werden
-- die Werte sollten am Ende der Transaktion also nicht mehr 0 sein, sondern denen des Produktes "Der Verkehrte Himmel" entsprechen
-- außerdem wird die Stückzahl des Produktes im Lager verändert:

SELECT * FROM produkt_im_lager WHERE fid_produkt = 2 AND fid_lager = 2;

 id_produkt_im_lager | fid_lager | fid_produkt | stueckzahl |     letzte_lieferung      
---------------------+-----------+-------------+------------+---------------------------
                   8 |         2 |           2 |          8 | 2024-09-20 13:42:01.86508
(1 row)
-- gegenwärtig sind nach meiner ganzen Wüterei in den Tabellen 8 Exemplare des Comics im Berliner Lager vorhanden. Es sollte nach der Transaktion eines weniger da sein.

BEGIN;

UPDATE bestellungen
SET gesamtpreisnetto = gesamtpreisnetto + (SELECT SUM(p.preis_netto * pos.menge)
    FROM bestellpositionen pos
    INNER JOIN produkte p ON pos.fid_produkt = p.id_produkt
    WHERE pos.fid_bestellung = 6 AND p.id_produkt = 2)
WHERE id_bestellung IN (
    SELECT best.id_bestellung
    FROM bestellpositionen pos
    INNER JOIN bestellungen best ON pos.fid_bestellung = best.id_bestellung
    INNER JOIN produkte p ON pos.fid_produkt = p.id_produkt
    WHERE best.id_bestellung = 6 AND p.id_produkt = pos.fid_produkt
);

UPDATE bestellungen
SET mwst = mwst + (SELECT SUM(p.mwst_betrag * pos.menge)
    FROM bestellpositionen pos
    INNER JOIN produkte p ON pos.fid_produkt = p.id_produkt
    WHERE pos.fid_bestellung = 6 AND p.id_produkt = 2)
WHERE id_bestellung IN (
...
UPDATE bestellungen
SET gesamtpreisbrutto = gesamtpreisbrutto + (SELECT SUM(p.preis_brutto * pos.menge)
    FROM bestellpositionen pos
    INNER JOIN produkte p ON pos.fid_produkt = p.id_produkt
    WHERE pos.fid_bestellung = 6 AND p.id_produkt = 2)
WHERE id_bestellung IN (
...
UPDATE produkt_im_lager 
SET stueckzahl = stueckzahl - (SELECT pos.menge
    FROM produkte p
    INNER JOIN produkt_im_lager pl ON p.id_produkt = pl.fid_produkt
    INNER JOIN bestellpositionen pos ON pos.fid_produkt = p.id_produkt
    WHERE pos.id_bestellposition = 6 LIMIT 1)
WHERE id_produkt_im_lager IN (
    SELECT pl.id_produkt_im_lager
    FROM produkt_im_lager pl
    INNER JOIN lager l ON l.id_lager = pl.fid_lager
    WHERE l.id_lager=2 AND pl.fid_produkt = 2
);

COMMIT;

-- natürlich habe ich den Code für die Transaktion nicht aus dem Ärmel geschüttelt. Es erforderte Einiges, bis er stand. Umso erfreulicher ist es, das in der Konsole zu lesen:
BEGIN
UPDATE 1
UPDATE 1
UPDATE 1
...
-- Ich wiederhole zwei Abfragen, um die Veränderungen zu sehen:

SELECT * FROM bestellungen WHERE fid_benutzer = 2 ORDER BY bestelldatum DESC LIMIT 1;

 id_bestellung |        bestelldatum        | fid_benutzer | gesamtpreisnetto | mwst | gesamtpreisbrutto 
---------------+----------------------------+--------------+------------------+------+-------------------
             6 | 2024-09-25 16:22:04.200982 |            2 |            26.17 | 1.83 |             28.00
(1 row)
...
SELECT * FROM produkt_im_lager WHERE fid_produkt = 2 AND fid_lager = 2;
 ...
 id_produkt_im_lager | fid_lager | fid_produkt | stueckzahl |     letzte_lieferung      
---------------------+-----------+-------------+------------+---------------------------
                   8 |         2 |           2 |          7 | 2024-09-20 13:42:01.86508
(1 row)

-- Der Preis ist nun eingetragen und die Stückzahl im Lager von Berlin um 1 verringert. Jetzt muss ich bald das Lager wieder auffüllen, wenn das so weiter geht ;)

```
## Die Spalte Einzugsgebiet
26.09.2024:
... oder besser gesagt, zwei Spalten. Wir haben zehn Gebiete mit Postleitzahlen in Deutschland, die je mit 0-9 anfangen. Das mache ich mir zunutze, indem ich in die Tabelle Lager zwei Spalten einfüge, die den niedrigsten Wert und den höchsten Wert markieren. Es gibt sicherlich andere Lösungen, doch für die Übung soll das jetzt reichen. 
Ich ordne also dem Lager in Berlin die Postleitzahlen zu, die mit 0 und 1 anfangen, Hamburg bekommt 2, Köln bekommt 3, 4 und 5, Frankfurt 6 und 7 und München 8 und 9.

```sql
-- Im Datentyp Lieferadresse und Rechnungsadresse ändere ich den Datentyp der Postleitzahl von CHAR zu SMALLINT, um die Zahlen in der Query als Zahlen behandeln zu können.
-- Dafür muss ich erst die Spalten in der Tabelle benutzer löschen, um Abhängigkeiten herauszunehmen
ALTER TABLE benutzer 
DROP COLUMN lieferadresse;

ALTER TABLE benutzer 
DROP COLUMN rechnungsadresse;


ALTER TYPE lieferadresse ALTER ATTRIBUTE "postleitzahl" SET DATA TYPE INT;

ALTER TYPE rechnungsadresse ALTER ATTRIBUTE "postleitzahl" SET DATA TYPE INT;

ALTER TABLE benutzer 
ADD COLUMN lieferadresse lieferadresse;

ALTER TABLE benutzer ALTER COLUMN lieferadresse SET NOT NULL;

ALTER TABLE benutzer 
ADD COLUMN rechnungsadresse rechnungsadresse;

-- Natürlich habe ich mir damit die Dummydaten ein bisschen zerschossen und füge neue ein

UPDATE benutzer SET lieferadresse = ROW('Martina Musterfrau', 'Musterstraße 1', 12345)
WHERE id_benutzer = 1;

UPDATE benutzer SET lieferadresse = ROW('Max Mustermann', 'Musterstraße 2', 16789)
WHERE id_benutzer = 2;


-- Neue Spalten! Endlich!
ALTER TABLE lager ADD COLUMN einzugsgebiet_min INT;

ALTER TABLE lager ADD COLUMN einzugsgebiet_max INT;

UPDATE lager SET einzugsgebiet_min = 00000, einzugsgebiet_max = 19999 WHERE standort = 'Berlin';
UPDATE lager SET einzugsgebiet_min = 20000, einzugsgebiet_max = 29999 WHERE standort = 'Hamburg';
UPDATE lager SET einzugsgebiet_min = 30000, einzugsgebiet_max = 59999 WHERE standort = 'Köln';
UPDATE lager SET einzugsgebiet_min = 60000, einzugsgebiet_max = 79999 WHERE standort = 'Frankfurt';
UPDATE lager SET einzugsgebiet_min = 80000, einzugsgebiet_max = 99999 WHERE standort = 'München';

world_of_comics=# SELECT * FROM lager;
 id_lager | standort  | kapazitaet_m2 | einzugsgebiet_min | einzugsgebiet_max 
----------+-----------+---------------+-------------------+-------------------
        2 | Berlin    |            37 |                 0 |             19999
        1 | Hamburg   |            20 |             20000 |             29999
        3 | Köln      |            25 |             30000 |             59999
        4 | Frankfurt |            20 |             60000 |             79999
        5 | München   |            18 |             80000 |             99999

-- keine Transaktion, aber eine Query. User 1 hat einen Manga zurückgegeben (id_produkt 1), und aus den Benutzerdaten erfahren wir die Postleitzahl.
-- die Nummer des Lagers könnte in der Anwendung mit einem Switch-Statement "einsortiert" werden und mit folgender Query in der Datenbank abgelegt:

-- erst mal gucken, wie viel Stück von dem Manga im entsprechenden Lager sind
SELECT * FROM produkt_im_lager 
WHERE fid_lager = (SELECT pl.fid_lager FROM produkt_im_lager pl
INNER JOIN lager l ON pl.fid_lager = l.id_lager
WHERE l.einzugsgebiet_min >= 0 AND l.einzugsgebiet_max <= 19999 LIMIT 1) AND fid_produkt = 1;

id_produkt_im_lager | fid_lager | fid_produkt | stueckzahl |     letzte_lieferung      
---------------------+-----------+-------------+------------+---------------------------
                   7 |         2 |           1 |         16 | 2024-09-20 13:42:01.86508

UPDATE produkt_im_lager SET stueckzahl = stueckzahl + 1
WHERE fid_lager = (SELECT pl.fid_lager FROM produkt_im_lager pl
INNER JOIN lager l ON pl.fid_lager = l.id_lager
WHERE l.einzugsgebiet_min >= 0 AND l.einzugsgebiet_max <= 19999 LIMIT 1) AND fid_produkt = 1;
...
WHERE l.einzugsgebiet_min >= 0 AND l.einzugsgebiet_max <= 19999 LIMIT 1) AND fid_produkt = 1;
 id_produkt_im_lager | fid_lager | fid_produkt | stueckzahl |     letzte_lieferung      
---------------------+-----------+-------------+------------+---------------------------
                   7 |         2 |           1 |         17 | 2024-09-20 13:42:01.86508


```
## ERD nachhher

Hier ist jetzt die endgültige Version meines Entity-Relationship-Diagramms, Stand 26.09.2024:

![physisches Entity-Relationship-Diagram nachher](/ERD-physisch-nachher.png)